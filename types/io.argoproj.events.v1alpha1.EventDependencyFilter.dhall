{ context :
    Optional
      ./io.argoproj.events.v1alpha1.EventContext.dhall
        sha256:e3900a05b96a52077606d768ada848ef4c90532a90c031d054866ec2da0717d5
, data :
    Optional
      ( List
          ./io.argoproj.events.v1alpha1.DataFilter.dhall
            sha256:ef3cd81b507acc56e1c2165119344441344a04b1a34afe3fe2e3c1348c277559
      )
, dataLogicalOperator : Optional Text
, exprLogicalOperator : Optional Text
, exprs :
    Optional
      ( List
          ./io.argoproj.events.v1alpha1.ExprFilter.dhall
            sha256:fabdbf256e08646c480bcc5d5e952bc11123574f04a1ac158dce989c9bbc14a7
      )
, script : Optional Text
, time :
    Optional
      ./io.argoproj.events.v1alpha1.TimeFilter.dhall
        sha256:08b2e66593f50b2f8d059b80a2fe8b010c866918df8e8f61f5c65eb42061459b
}
